// Provided BARCode function to generate barcode
function BARCode( B ) {
...
// Event listener for generating barcode
document.getElementById('generate-btn').addEventListener('click', function() {
	var inputText = document.getElementById('barcode-input').value.trim();
	...
	// Clear previous barcode and error messages
	barcodeContainer.innerHTML = '';
	barcodeText.innerHTML = '';
	errorMessage.innerHTML = '';

	if (inputText === '') {
		errorMessage.innerHTML = 'Please enter text for the barcode.';
		return;
	}

	// Generate and display the barcode
	try {
		var barcodeSvg = BARCode(inputText);
		barcodeContainer.appendChild(barcodeSvg);
		barcodeText.innerHTML = 'Barcode for: ' + inputText;
	} catch (e) {
		errorMessage.innerHTML = 'Error generating barcode. Please try again.';
		console.error(e);
	}
});